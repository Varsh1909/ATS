...
app.post('/rank-candidates', async (req, res) => {
  ...
  console.log(`Received request for role: ${requiredRole}, topN: ${topN}`);
  console.log(`Date filter: Start Date = ${startDate}, End Date = ${endDate}`);
  console.log(`Experience range: Min = ${minExperience}, Max = ${maxExperience}`);
  console.log(`Mandatory Skills: ${mandatorySkills}`);
  console.log(`Optional Skills: ${optionalSkills}`);
  console.log('Fetching test data from [dbo].[vw_export_Candidate] table...');
  ...
  try {
    // Fetch test data from SQL Server
    const testData = await executeQuery('SELECT *, CAST(created_at AS DATE) AS date FROM [dbo].[vw_export_Candidate]');
    ...
    const response = await axios.post('http://localhost:5000/rank-candidates', payload, {
      timeout: 90000 // Optional: Set a timeout for the request
    });
